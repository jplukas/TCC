...
class Searcher:
    ...
    def init_collection(self, edition):
        is_loaded = False
        try:
            collection =  self.client.get_collection(self.coll_base_name + '__' + edition + '_2')
            is_loaded = True
        except:
            collection = self.client.create_collection(self.coll_base_name + '__' + edition + '_2')

        return is_loaded, collection


    def insert_db(self, batch_ids, batch_txt, collection, metadatas):
        embeddings = self.embedder.encode(batch_txt, normalize_embeddings=True, task="separation", prompt_name="separation")
