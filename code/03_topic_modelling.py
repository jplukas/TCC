...
def createTopicModel(embedder: str, granularity: granType, clustering: dict[str, Any], vectorizer_model, ctfidf_model, n_topics: int,
...
if __name__ == "__main__":
    ...
    for conf in tqdm(iterConfigs()):
        ...
        if not os.path.exists(model_filepath):
                topic_model = createTopicModel(**{"vectorizer_model": vectorizer_model, "ctfidf_model":ctfidf, **conf})
                topic_model.save(model_filepath)
            
        else:
            topic_model = BERTopic.load(model_filepath)
        # topic_models[model_filepath] = topic_model