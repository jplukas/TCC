...
\usepackage{wrapfig}
\usepackage{lipsum}
\tcbposterset{fontsize = 21pt} % default, mude se necessário
...
\posterbox[adjusted title = 2. Redução de dimensionalidade: UMAP,smallmargins]{name=umap, below=sem_map, column=1, span=6}{
  ...
  Os modelos de embedding tipicamente projetam sentenças em espaços de alta dimensionalidade (\(d = 1000\), tipicamente). Devido à \textbf{maldição da dimensionalidade}, aplicamos uma técnica de redução de dimensionalidade denominada UMAP~\parencite{mcinnes2018umap}, para viabilizar a etapa posterior de \textit{clustering} (e também para visualização).\\
  \begin{figure}[H]
    \begin{subfigure}[c]{.40\textwidth}
...
\posterbox[adjusted title = 3. \textit{Clustering}: HDBSCAN, smallmargins]{name=hdbscan, below=umap, column=1, span=6}{
  ...
  \begin{figure}[H]
    \begin{subfigure}[c]{.40\textwidth}
      ...
      \caption{Clusters (coloridos) de diferentes formas e densidades. Observe a presença de pontos classificados como ruído}
    \end{subfigure}
    \begin{subfigure}[c]{.40\textwidth}
